...
-- Modificamos las columnas en personal_info para que sean de tipo TEXT
ALTER TABLE personal_info
...
-- Modificamos la columna academic_modality en education para que sea de tipo TEXT
-- Nota: No modificamos institution_id porque no existe, usamos institution en su lugar
ALTER TABLE education
...
-- Nota: No añadimos la restricción para education.institution_id porque no existe esa columna
ALTER TABLE education
...
-- Añadir triggers para actualizar timestamps
...
CREATE TRIGGER update_marital_status_timestamp
BEFORE UPDATE ON marital_status
FOR EACH ROW EXECUTE FUNCTION update_timestamp();

CREATE TRIGGER update_academic_modalities_timestamp
BEFORE UPDATE ON academic_modalities
FOR EACH ROW EXECUTE FUNCTION update_timestamp();
