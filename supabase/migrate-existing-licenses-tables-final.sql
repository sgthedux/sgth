-- Migración para actualizar tablas existentes de licencias (VERSIÓN FINAL CORREGIDA)
...
CREATE POLICY "Los usuarios pueden ver evidencias de sus propias solicitudes"
    ...
    USING (
        EXISTS (
            ...
            WHERE license_requests.id = license_requests.id 
...
-- Políticas para administradores (usando roles nativos de Supabase)
...
-- Generar radicados para registros existentes que no los tengan (CORREGIDO)
DO $$
DECLARE
    record_row RECORD;
    counter INTEGER := 1;
    ...
    new_radicado TEXT;
BEGIN
    -- Obtener el año actual o el año de creación más reciente
    SELECT TO_CHAR(COALESCE(MAX(fecha_creacion), MAX(created_at), NOW()), 'YYYY') 
    INTO year_part 
    FROM license_requests;
    
    -- Actualizar registros sin radicado uno por uno
    FOR record_row IN 
        SELECT id FROM license_requests 
        WHERE radicado IS NULL OR radicado = ''
        ORDER BY COALESCE(fecha_creacion, created_at, NOW())
    LOOP
        -- Generar radicado único
        LOOP
            new_radicado := 'LIC-' || year_part || '-' || LPAD(counter::TEXT, 6, '0');
...
DO $$
BEGIN
    RAISE NOTICE 'Migración de tablas de licencias completada exitosamente';
    RAISE NOTICE 'Tablas afectadas: license_requests, license_evidences';
    RAISE NOTICE 'Ninguna otra tabla fue modificada';
END $$;
